...
class SuperUser(commands.GroupCog, group_name=settings.sudo_group_name):
    ...
    async def give(
        ...
        for i in range(amount):
            instance = await CarInstance.create(
                ...
                horsepower_bonus=(
                    horsepower_bonus if horsepower_bonus is not None else random.randint(-20, 20)
                ),
                weight_bonus=(
                    weight_bonus if weight_bonus is not None else random.randint(-20, 20)
                ),
                event=event,
            )
        await interaction.followup.send(
    ...
    async def rebirth_add(
        ...
        await interaction.followup.send(f"Successfully added {amount} rebirth{plural} to {user.name}.")
    ...
    async def rebirth_remove(
        ...
        await interaction.followup.send(f"Successfully removed {amount} rebirth{plural} from {user.name}.")
